...
CREATE TABLE IF NOT EXISTS `sportssocialmediadb`.`post_comments` (
  ...
  `user_post_user_id` INT NOT NULL,
  `created_at` DATETIME NOT NULL,
  PRIMARY KEY (`id`, `user_post_id`, `user_post_user_id`),
  INDEX `fk_PostComments_user_post1_idx` (`user_post_id` ASC, `user_post_user_id` ASC) VISIBLE,
  CONSTRAINT `fk_PostComments_user_post1`
    FOREIGN KEY (`user_post_id` , `user_post_user_id`)
    REFERENCES `sportssocialmediadb`.`user_post` (`id` , `user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
